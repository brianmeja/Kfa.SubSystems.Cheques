...
namespace Kfa.SubSystems.Cheques.Contracts.Behaviors
{
    ...
    /// </summary>
    public sealed class AutoCompleteBoxCurrencyBehavior : Behavior<AutoCompleteBox>
    {
        /// <summary>
        ...
        private void LostFocus(object sender, RoutedEventArgs e)
        {
            try
            {
                if (AssociatedObject != null)
                {
                    if (decimal.TryParse(new string(AssociatedObject.Text?
                        .Where(c => char.IsDigit(c) || c == '.').ToArray()), out decimal ans))
