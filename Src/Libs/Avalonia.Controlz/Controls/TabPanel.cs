...
namespace Avalonia.Controlz.Controls
{
    ...
    {
        int _numRows = 1;       // Nubmer of row calculated in measure and used in arrange
        int _numHeaders = 0;    // Number of headers excluding the collapsed items
        double _rowHeight = 0;  // Maximum of all headers height
        ...
        Dock TabStripPlacement
        {
            get
            {
                ...
                if (tc != null) placement = tc.TabStripPlacement;
                return placement;
            }
        }
        ...
        Size GetDesiredSizeWithoutMargin(IControl element)
        {
        ...
        int GetActiveRow(int[] solution)
        {
            ...
            if (solution.Length > 0)
            {
                foreach (IControl child in Children)
                {
                    ...
                    bool? isActiveTab = (child as ISelectable)?.IsSelected;

                    if (isActiveTab == true) return activeRow;
        ...
        */
        ...
        int[] CalculateHeaderDistribution(double rowWidthLimit, double[] headerWidth)
        {
